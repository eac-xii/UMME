<template>
  <div class="searchBox d-flex align-items-center">
    ...
    <div class="dropdown searchDrop">
      <input id="searchInput" type="text" class="dropdown-toggle"
      data-bs-toggle="dropdown" aria-expanded="false"
      placeholder="What do you want to play?" autocomplete="off"
      @keyup="search" v-model.trim="query">
        ...
        <li v-for="item in dropdownItems" :key="item.data.id" class="my-1">
          <a href="#" class="dropdown-item">
            <div v-if="item.type === 'artist'" class="d-flex align-items-center">
              ...
              <button class="ms-auto btn btn-outline-secondary border-0 rounded-circle searchByArtistBtn d-flex justify-content-center align-items-center" @click.prevent="searchArtistTracks(item.data.id)">
            ...
            <div v-if="item.type === 'track'" class="d-flex align-items-center">
              ...
              <div class="trackDetail mx-2 d-flex flex-column">
                <p class="itemName">{{ item.data.name }}</p>
                <p class="trackArtists"><span class="itemType">{{ item.type.toUpperCase() }} · </span>{{ item.data.artists.map(artist => artist.name).join(', ') }}</p>
              </div>
              <button class="ms-auto btn btn-outline-secondary border-0 rounded-circle addTrackToPlaylistBtn d-flex justify-content-center align-items-center" @click.prevent="addTrackToPlaylist(item.data)">+</button>
            </div>
          </a>
        </li>
      </ul>
...
.searchByArtistBtn, .addTrackToPlaylistBtn {
  width: 2vh;
  height: 2vh;
}
